...
State <-"WA"
State_folder <- "wa/"
State_poa <- c(as.character(6000:6797),"0872")



wiki_page <- read_html("https://en.wikipedia.org/wiki/Local_government_areas_of_Western_Australia")


lgas_list <- map_dfr(1:2, function(x,wiki_page){
  ...
  region_exist <- "Region" %in% colnames(a)
  
  if(region_exist){
    a %>% select(LGA,Region) %>% 
      rename(State.Region=Region) %>%
    ...
    a %>% select(LGA) %>%
      mutate(ABB_NAME=toupper(LGA),
             Metro.Region="",
             State.Region="Metropolitan Perth")
...
shp_files <- tribble(~name,~filename,~url,
                     "LGA","WA_LGA_POLYGON_SHP_GDA2020.shp","https://data.gov.au/data/dataset/8a8e0037-e474-422f-8026-241c7c88551f/resource/9e15d69c-c0bc-49eb-af47-9bb5f71bb6ae/download/wa_lga_polygon_shp_gda2020.zip",
                     "LOC","WA_LOCALITY_POLYGON_SHP_GDA2020/WA_LOCALITY_POLYGON_SHP_GDA2020.shp","https://data.gov.au/data/dataset/6a0ec945-c880-4882-8a81-4dbcb85e74e5/resource/9fff5439-7af5-42f4-9102-42c4199c5c1c/download/wa_locality_polygon_shp_gda2020.zip",
...
#shapes$LGA <- shapes$LGA %>% rename(LGA_NAME=WA_LGA__2)
shapes$LOC <- shapes$LOC %>% rename(NAME=WA_LOCAL_2)
...
a<- loc_lga %>% filter(grepl("Metropolitan Perth",State.Region)) 
a$area <- st_area(a)
plot(a %>% select(area))
...
### check missing WAs -- in WA there are four WAs for offshore and no usual address
