...
State <-"QLD"
State_folder <- "qld/"
State_poa <- c(4000:4999,2406)
State_poa <- as.character(State_poa)



wiki_page <- read_html("https://en.wikipedia.org/wiki/Local_government_areas_of_Queensland")
...
shp_files <- tribble(~name,~filename,~url,
                     "LGA","QLD_LGA_POLYGON_shp_GDA2020/QLD_LGA_POLYGON_shp_GDA2020.shp","https://data.gov.au/data/dataset/16803f0b-6934-41ae-bf82-d16265784c7f/resource/cbd745be-0330-4a1b-872f-6c87e62bcfef/download/qld_lga_polygon_shp_gda2020.zip",
                     "LOC","QLD_LOC_POLYGON_shp_GDA2020/QLD_LOC_POLYGON_shp_GDA2020.shp","https://data.gov.au/data/dataset/6bedcb55-1b1f-457b-b092-58e88952e9f0/resource/d20d0a54-7680-43c4-8c46-a08e3bc43fa0/download/qld_loc_polygon_shp_gda2020.zip",
...
shapes$LGA <- shapes$LGA %>% rename(LGA_NAME=QLD_LGA__2)
shapes$LOC <- shapes$LOC %>% rename(NAME=QLD_LOCA_2)
...
plot(loc_lga %>% filter(grepl("South East Queensland",State.Region)) %>% select(LOCALITY))
...
### check missing SAs -- in QLD there are four SAs for offshore and no usual address
